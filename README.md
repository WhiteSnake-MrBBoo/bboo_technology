...
- “이 결과 저장” 버튼 → `ocr_gpt_result` 테이블에 저장 → 히스토리/엑셀로 재활용
...
- **AI 모델 역할 분리(application.yml 기반)**  
  ...
  - 예)  
    - 반복 호출이 많은 요약/마케팅 포인트 → `gpt-4.1-mini / gpt-3.5-turbo` 등 **저비용 모델**  
...
- **토큰 비용 절감 구조(Usage 로깅 + 재사용)**  
  ...
  - 이를 통해:
    - 상품/기간/모델별 **토큰 사용량과 비용을 수치로 확인**  
    - “많이 나가는 구간”을 찾아 프롬프트/모델을 조정하는 **피드백 루프** 구축  
  - 이미 생성된 결과는 `ocr_gpt_result`에서 재사용하므로,  
    **같은 상품에 대해 불필요하게 AI를 반복 호출하지 않도록** 설계

- **프롬프트 구조화 & 응답 길이 제어를 통한 비용 관리**  
  - 요약/포인트는 **항목형·리스트형 출력**으로 제한해  
    불필요하게 긴 답변을 줄이고, completion 토큰을 자연스럽게 절감  
  - 상품기술서(OCR 원문)는 **전체 내용을 기준으로** 처리하되,  
    헤더·푸터·광고성 문구 등 노이즈만 사전 정제해 **핵심 정보는 그대로 유지**  
  - 필요 시 **“섹션 단위 요약 → 최종 통합 요약”** 방식으로,  
    내용을 훼손하지 않고 단계적으로 처리하면서도 토큰 사용량을 관리할 수 있도록 설계

- **설정 주도형 아키텍처**  
  - API 키, Base URL, 기본 모델, 목적별 모델명, temperature 등을  
    모두 `application.yml` + `OpenAiConfig`에서 관리  
...
- **공통 GPT 서비스 계층**  
  ...
  - 나중에 로컬 LLM / 사내 LLM으로 교체하더라도  
    **서비스 인터페이스만 맞추면 기존 비즈니스 로직을 그대로 재사용**할 수 있도록 설계

> 🤖 프롬프트 설계, 모델 설정, WebClient 호출 구조:  
> [GPT AI Module](./docs/gpt-ai-module.md)
