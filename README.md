...
- **모델 역할 분리(application.yml 기반)**  
  - `summary`, `hostScript`, `marketingPoints` 용도를 분리해서  
    동일 프로젝트 안에서도 **역할별로 다른 모델/파라미터**를 사용할 수 있게 설계  
  - 예) 요약/마케팅 포인트는 `gpt-4.1-mini / gpt-3.5-turbo`로 비용 최적화,  
    쇼호스트 멘트는 향후 `gpt-4.x / gpt-5.x` 계열로 교체 가능하도록 구성

- **비용(토큰) 관리 가능 구조**  
  - OpenAI 응답의 `prompt_tokens / completion_tokens / total_tokens`를  
    `OcrGptResult` 엔티티에 함께 저장할 수 있도록 설계  
  - 엑셀 내보내기 시 “토큰 사용량 포함” 옵션을 두어,  
    **상품별·기간별 AI 사용량 분석 및 비용 추적**이 가능하도록 확장 여지 확보

- **설정 주도형 아키텍처**  
  - API 키, Base URL, 기본 모델, 목적별 모델명, temperature 등을  
    모두 `application.yml` + `OpenAiConfig`에서 관리  
  - 코드 수정 없이 **설정만으로 모델 교체·튜닝**이 가능해  
...
- **공통 GPT 서비스 계층**  
  ...
  - 나중에 로컬 LLM / 사내 LLM으로 교체하더라도  
    **서비스 인터페이스만 맞추면 백엔드 전반을 재사용**할 수 있도록 설계
